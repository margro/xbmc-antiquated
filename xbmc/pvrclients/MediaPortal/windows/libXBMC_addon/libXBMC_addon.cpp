...
cHelper_libXBMC_addon::cHelper_libXBMC_addon()
{
  m_Handle       = NULL;
  m_cb           = NULL;
}
...
void cHelper_libXBMC_addon::Log(const addon_log_t loglevel, const char *format, ... )
{
  if (m_cb == NULL)
    return;
...
bool cHelper_libXBMC_addon::GetSetting(std::string settingName, void *settingValue)
{
  if (m_cb == NULL)
    return false;
...
void cHelper_libXBMC_addon::QueueNotification(const queue_msg_t type, const char *format, ... )
{
  if (m_cb == NULL)
    return;
...
void cHelper_libXBMC_addon::UnknownToUTF8(std::string &str)
{
  if (m_cb == NULL)
    return;
